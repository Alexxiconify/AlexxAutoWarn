  ...
  if ( handItem != null && handItem.hasItemMeta ( ) ) {
   ...
   if ( meta.getPersistentDataContainer ( ).has ( wandKey , PersistentDataType.STRING ) ) {
    ...
    if ( event.getAction ( ) == Action.LEFT_CLICK_BLOCK ) {
     plugin.getAutoWarnCommand ( ).setPos1 ( player.getUniqueId ( ) , clickedBlockVector ); // Pass Vector
     ...
     plugin.getAutoWarnCommand ( ).setPos2 ( player.getUniqueId ( ) , clickedBlockVector ); // Pass Vector
     player.sendMessage ( settings.getMessage (
